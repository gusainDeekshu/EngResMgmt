...
const GenerateTicketModal = ({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Generate Ticket for #{order.confirmationId}</DialogTitle>
        </DialogHeader>
        <div className="grid gap-4 py-4">
          {isLoadingVenues ? (
            <div className="text-center text-muted-foreground">Loading venues...</div>
          ) : modalError ? (
            <div className="text-center text-destructive">{modalError}</div>
          ) : venues.length === 0 ? (
            <div className="text-center text-muted-foreground">No venues available for this event.</div>
          ) : (
...
export default function TicketManagementPage() {
  ...
  useEffect(() => {
    fetchOrders();
  }, [fetchOrders, filters.search, pagination.currentPage, filters.status]);

  const handleAction = async (action: "send_email_ticket" | "send_whatsapp_ticket", orderId: string) => {
    if (!confirm(`Are you sure you want to send this ticket via ${action.includes("email") ? "Email" : "WhatsApp"}?`))
      return;
    try {
                ...
                {orders.map((order) => (
                  <TableRow key={order._id}>
                    <TableCell>
                      ...
                      <div className="text-xs text-muted-foreground">#{order.confirmationId}</div>
                    </TableCell>
                    <TableCell>
                      ...
                      {order.fulfillmentStatus === "ticket_sent" && order.ticketDetails?.ticketNumber ? (
                        <div>
                          ...
                          <div className="text-xs text-muted-foreground">
                            {order.ticketDetails.selectedVenue?.name || "Venue TBD"},{" "}
                            {order.ticketDetails.selectedVenue?.city || ""}
                          </div>
                          <div className="text-xs text-muted-foreground">
                            {order.ticketDetails.selectedDate
                              ? new Date(order.ticketDetails.selectedDate).toLocaleDateString()
                              : "Date TBD"}{" "}
                            at {order.ticketDetails.eventTime || "Time TBD"}
                          </div>
                        </div>
                      ) : (
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <Button
                          ...
                          disabled={order.fulfillmentStatus === "ticket_sent"}
                        >
                        ...
                        <Button
                          ...
                          disabled={order.fulfillmentStatus !== "ticket_sent" || isDownloading === order._id}
                        >
                        ...
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => handleAction("send_email_ticket", order._id)}
                          disabled={order.fulfillmentStatus !== "ticket_sent"}
                        >
                        ...
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => handleAction("send_whatsapp_ticket", order._id)}
                          disabled={order.fulfillmentStatus !== "ticket_sent"}
                        >
