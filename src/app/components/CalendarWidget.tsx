...
export default function CalendarTimeline({
  ...
  const engineers = Array.from(
    new Set(assignments.map((a) => a.engineer).filter(Boolean))
  );
    ...
    <Card className="p-4">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 mb-2">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <Button size="sm" onClick={handleAddAssignment} variant="outline">
            + Add Assignment
          </Button>
        </div>
      </div>
      <div className="overflow-x-auto">
        <div className="flex border-b text-xs text-gray-500 min-w-fit">
          ...
          {days.map((d, i) => (
            <div
              key={i}
              className={`w-7 md:w-8 text-center ${
        ...
        {visibleAssignments.map((a) => {
          ...
          return (
            <div
              key={a._id}
              className="flex items-center border-b last:border-0 hover:bg-gray-50 transition cursor-pointer"
            ...
            >
              <div className="w-40 pr-2 py-2 text-sm font-medium text-gray-700 flex items-center gap-2">
                ...
                <div>
                  ...
                  {a.engineer && (
                    <div className="text-xs text-gray-400">{a.engineer}</div>
                  )}
                </div>
              </div>
                ...
                <div className="flex w-full">
                  {days.map((d, i) => {
                    ...
                    return (
                      <div
                        key={i}
                        className={`w-7 md:w-8 h-4 mx-0.5 rounded ${
                          isBar ? color : ""
                        } ${isToday(d) ? "border-2 border-indigo-600" : ""} ${
                        ...
                        title={
                          isBar
                            ? `${a.project} | ${a.role}${
                              ...
                              }\n${format(start, "yyyy-MM-dd")} to ${format(
                                end,
                                "yyyy-MM-dd"
                              )}`
                            : format(d, "yyyy-MM-dd")
                        ...
                        style={{
                          opacity: isBar ? 0.9 : 1,
                          cursor: isBar ? "pointer" : undefined,
                        }}
                      ></div>
                    );
                  })}
      ...
      <Dialog
        ...
        onOpenChange={(v) => !v && setModalAssignment(null)}
      >
        <DialogContent>
          <DialogHeader>
          ...
          {modalAssignment && (
            <div className="space-y-2">
              <div>
                <b>Project:</b> {modalAssignment.project}
              </div>
              <div>
                <b>Role:</b> {modalAssignment.role}
              </div>
              {modalAssignment.engineer && (
                <div>
                  <b>Engineer:</b> {modalAssignment.engineer}
                </div>
              )}
              <div>
                <b>Start:</b>{" "}
                {format(new Date(modalAssignment.start), "yyyy-MM-dd")}
              </div>
              <div>
                <b>End:</b>{" "}
                {format(new Date(modalAssignment.end), "yyyy-MM-dd")}
              </div>
              <div>
                <b>Allocation:</b> {modalAssignment.allocation}%
              </div>
            </div>
          )}
        </DialogContent>
